...
class Printer:
    ...
    def update(self):
        draw_speed = 0.1
        screen_dist = 100
        to_center = LineSegment(
            Point(
                self.circle_center_point.x
                + self.circle_radius * math.cos(self.circle_angle),
                self.circle_center_point.y,
            ),
            self.screen_center_point,
        )
        circle_point = to_center.get_point(
            self.circle_radius * math.sin(self.circle_angle) / screen_dist
        )
        if not (
            circle_point.x < 0
            or circle_point.x > pyxel.width
            or circle_point.y < 0
            or circle_point.y > pyxel.height
        ):
...
class Painter:
    ...
    def update_painting(self, point):
        ...
        if len(self.painting) >= 1:
            if self.painting[-1].x == point.x and self.painting[-1].y == point.y:
                return
            if (
                len(self.painting) > self.min_painting_num
                and Utils.get_distance(self.painting[0], point) < 5
            ):
    ...
    def update_printers(self, laser_points):
        if self.printing_cnt < len(self.painting) - 1:
            if (
                self.painting[self.printing_cnt].y
                == self.painting[self.printing_cnt + 1].y
            ):
                circle_center_point = Point(
                    (
                        self.painting[self.printing_cnt].x
                        + self.painting[self.printing_cnt + 1].x
                    )
                    / 2,
                    self.painting[self.printing_cnt].y,
                )
                circle_radius = (
                    Utils.get_distance(
                        self.painting[self.printing_cnt],
                        self.painting[self.printing_cnt + 1],
                    )
                    / 2.0
                )
...
class Laser:
    def __init__(self, point):
        ...
        self.laser_angle = []
        for i in range(self.laser_num):
            self.laser_angle.append(
                random.random() * math.pi / self.laser_num
                + i * math.pi / self.laser_num
            )
    ...
    def update(self):
        for i in range(self.laser_num):
            self.laser_angle[i] = (self.laser_angle[i] + self.laser_speed[i]) % (
                2 * math.pi
            )

    def draw(self):
        for i in range(self.laser_num):
            laser_line_1 = self.get_laser_line(
                self.laser_radius[i], self.laser_angle[i]
            )
            laser_line_2 = self.get_laser_line(
                self.laser_radius[i], self.laser_angle[i] + math.pi
            )
            pyxel.line(
                laser_line_1.get_start().x + self.center_point.x,
                laser_line_1.get_start().y + self.center_point.y,
                laser_line_1.get_end().x + self.center_point.x,
                laser_line_1.get_end().y + self.center_point.y,
                self.laser_color[i],
            )
            pyxel.line(
                laser_line_2.get_start().x + self.center_point.x,
                laser_line_2.get_start().y + self.center_point.y,
                laser_line_2.get_end().x + self.center_point.x,
                laser_line_2.get_end().y + self.center_point.y,
                self.laser_color[i],
            )
    ...
    def get_laser_line(self, radius, angle):
        return LineSegment(
            Point(
                radius * math.cos(angle + math.pi), radius * math.sin(angle + math.pi)
            ),
...
class Mouse:
    ...
    def update(self, color):
        ...
        for i in range(self.thunder_num):
            ...
            self.thunder_seg.append(
                Point(
                    point_on_line.x + random.randint(-noise, noise),
                    point_on_line.y + random.randint(-noise, noise),
                )
            )
    ...
    def draw(self):
        ...
        for i in range(len(self.thunder_seg) - 1):
            pyxel.line(
                self.thunder_seg[i].x,
                self.thunder_seg[i].y,
                self.thunder_seg[i + 1].x,
                self.thunder_seg[i + 1].y,
                self.thunder_color,
            )

        # draw cross
        cross_line_1 = self.get_cross_line(self.cross_radius, self.cross_angle)
        cross_line_2 = self.get_cross_line(
            self.cross_radius, self.cross_angle + math.pi / 2
        )
        pyxel.line(
            cross_line_1.get_start().x + pyxel.mouse_x,
            cross_line_1.get_start().y + pyxel.mouse_y,
            cross_line_1.get_end().x + pyxel.mouse_x,
            cross_line_1.get_end().y + pyxel.mouse_y,
            self.cross_color,
        )
        pyxel.line(
            cross_line_2.get_start().x + pyxel.mouse_x,
            cross_line_2.get_start().y + pyxel.mouse_y,
            cross_line_2.get_end().x + pyxel.mouse_x,
            cross_line_2.get_end().y + pyxel.mouse_y,
            self.cross_color,
        )
